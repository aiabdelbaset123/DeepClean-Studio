...
class AcademicRevisionEngine:
    def __init__(
        ...
        self.formulaic_rewrites = [
            ...
            (r"\bin today's (?:world|society)\b", "in current scholarship"),
            (r"\bplays a (?:crucial|vital) role in\b", "is important to"),
        ...
        self.editorial_vocabulary_swaps = [
            (r"\bshows important results\b", "reveals meaningful findings"),
            (r"\bshows that\b", "brings into view"),
            (r"\bimportant\b", "revealing"),
            (r"\bsignificant\b", "telling"),
            (r"\bshows\b", "reveals"),
            (r"\bindicates\b", "points toward"),
    ...
    def _english_editorial_pass(self, paragraph: str) -> str:
        ...
        for index, sentence in enumerate(sentences):
            ...
            if index == 0 and len(tokenize_words(sentence)) > 5:
                opener = self.editorial_voice_openers[index % len(self.editorial_voice_openers)]
                already_voiced = sentence.lower().startswith(tuple(item.lower() for item in self.editorial_voice_openers))
                if not already_voiced and not sentence.startswith("I "):
                    sentence = f"{opener}, {sentence[:1].lower()}{sentence[1:]}"
            if sentence and sentence[-1] not in ".!?":
                sentence += "."
            revised.append(sentence)

            if self.intensity >= 5 and len(tokenize_words(sentence)) >= 20 and index % 3 == 0:
                revised.append(self.editorial_short_beats[index % len(self.editorial_short_beats)])

        if self.intensity >= 4 and not self.editorial_question_added and len(revised) >= 3:
            midpoint = max(1, len(revised) // 2)
            revised.insert(midpoint, "Why should the reader settle for a flat report when the idea itself has movement?")
            self.editorial_question_added = True
    ...
    def _avoid_expansion(self, text: str) -> str:
        ...
        if not original_count or revised_count <= original_count * 1.15:
            return text
        text = re.sub(
            r"\b(Furthermore|Additionally|Moreover|However|Therefore|Notably|Clinically|Historically|In my reading|From close inspection|What stands out to me|As I see it),\s+",
            "",
            text,
        )
        text = re.sub(
            r"(?:That matters|Here is the turn|The point is sharp|Nothing about this is neutral)\.",
            "",
            text,
        )
        text = re.sub(
            r"Why should the reader settle for a flat report when the idea itself has movement\?",
            "",
            text,
        )
    ...
    def run(self) -> str:
        text = normalize_spacing(self.original_text)
...
class ArabicEditorialRevisionEngine:
    ...
    def _enrich_vocabulary(self, sentence: str, index: int) -> str:
        ...
        for pattern, replacement in self.vocabulary_swaps:
            ...
            if re.search(pattern, sentence):
                ...
                changes += 1
        if index == 0 and self.intensity >= 2:
            opener = self.voice_openers[index % len(self.voice_openers)]
            if not sentence.startswith(tuple(self.voice_openers)):
    ...
    def _shape_rhythm(self, sentences: List[str]) -> List[str]:
        ...
        for index, sentence in enumerate(sentences):
            ...
            should_add_beat = self.intensity >= 4 and words_count >= 18 and index % 3 == 0
            if should_add_beat:
                shaped.append(self.short_beats[index % len(self.short_beats)])
        return shaped

    def _add_mid_text_question(self, sentences: List[str]) -> List[str]:
        if self.question_added or self.intensity < 3 or len(sentences) < 3:
            return sentences
        question = "أليس هذا ما يجعل القراءة أعمق من مجرد تلقي معلومات؟"
        midpoint = max(1, len(sentences) // 2)
        sentences.insert(midpoint, question)
